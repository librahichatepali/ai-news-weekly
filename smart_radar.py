...
def ai_summarize(content, source_name):
    ...
    # 修改提示词：禁止 AI 判定是否有价值，要求其作为翻译官
    prompt = f"""
    任务：你是一个专业游戏行业情报员。请将来自 {source_name} 的新闻标题翻译成中文。
    要求：
    1. 简洁明了，直接以列表形式输出翻译结果。
    2. 不得回复“今日无重大更新”或“暂无新内容”。
    数据：
    {content}
    """
    ...
    for src in TARGET_SOURCES:
        try:
            ...
            for it in items:
                ...
                raw_feed_text += f"- {title}\n"
            
            if raw_feed_text:
                summary = ai_summarize(raw_feed_text, src['name'])
                if summary:
                    # 将换行符提前转换，规避 f-string 中的反斜杠错误
